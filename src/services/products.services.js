  ...
  async postRealTimeProducts(req, res) {
    try {
      ...
      if (req.body.borrar) {
        try {
          const productId = req.body.borrar;
          const product = await productModel.findById(productId); // Usa findById para encontrar un producto por su ID.
          const { title, _id, userId } = product;
          const user = await userModel.findById(userId); // Usa findById para encontrar un usuario por su ID.
          const { email, first_name, last_name } = user;
          await productModel.deleteOne({ _id: productId });
      ...
      console.error(error); // Registra el error en la consola.
      // Puedes manejar el error o enviar una respuesta de error al cliente si es necesario.
